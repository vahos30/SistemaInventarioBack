namespace SistemaInventario.Application.Feactures.Facturas
{
    ...
    public class CrearNotaCreditoFactusCommandHandler : IRequestHandler<CrearNotaCreditoFactusCommand, string>
    {
        ...
        public async Task<string> Handle(CrearNotaCreditoFactusCommand request, CancellationToken cancellationToken)
        {
            ...
            if (factura == null)
                throw new KeyNotFoundException("Factura no encontrada."); // <-- Cambia Exception por KeyNotFoundException
            ...
            // Validación para evitar desreferencia de null
            if (factusResponse?.data?.credit_note == null)
                throw new InvalidOperationException("La respuesta de Factus no contiene información de nota crédito.");
